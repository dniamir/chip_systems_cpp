...
bool MAX17260::check_por() {
    return (bool)MAX17260::read_field16(0x00);
}
...
void MAX17260::configure_system() {
    ...
    if (!MAX17260::check_por()) {
        while(MAX17260::read_field16(0x3D) & 1) {delay(10);}
    }

    uint16_t HibCFG = MAX17260::read_field(0xBA);
    MAX17260::write_field16(0x60, 0x90); // Exit Hibernate Mode step 1
    MAX17260::write_field16(0xBA, 0x0); // Exit Hibernate Mode step 2
    MAX17260::write_field16(0x60, 0x0); // Exit Hibernate Mode step 3

    MAX17260::write_field16(0x18, DesignCap); // Write DesignCap
    MAX17260::write_field16(0x1E, IchgTerm); // Write IchgTerm
    MAX17260::write_field16(0x3A, VEmpty); // Write VEmpty
    if (ChargeVoltage > 4.275) {
        MAX17260::write_field16(0xDB, 0x8400); // Write ModelCFG
    }
    else {
        MAX17260::write_field16(0xDB, 0x8000); // Write ModelCFG
    }
    //Poll ModelCFG.Refresh(highest bit),
    //proceed to Step 3 when ModelCFG.Refresh=0.
    while (MAX17260::read_field16(0xDB) & 0x8000) {delay(10);}

    MAX17260::write_field16(0xBA, HibCFG); // Restore Original HibCFG value

    // Clear POR Bit
    uint16_t Status = MAX17260::read_field16("Status");
    MAX17260::write_field16(0x00, Status & 0xFFFD);
...
float MAX17260::read_level_percent() {
    return MAX17260::read_field16(0x06) * MAX17260::per_per_lsb;
}

float MAX17260::read_level_mahrs() {
    return MAX17260::read_field16(0x05) * MAX17260::mah_per_lsb;
}

float MAX17260::read_tte() {
    return MAX17260::read_field16(0x11) * MAX17260::sec_per_lsb;
}
